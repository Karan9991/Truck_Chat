...
Future<List<Conversation>> getStoredConversations() async {
  ...
  List<String> encodedConversations = prefs.getStringList('conversations') ?? [];

  List<Conversation> conversations = encodedConversations.map((encodedConversation) {
    Map<String, dynamic> conversationMap = json.decode(encodedConversation);
    return Conversation(
      conversationId: conversationMap['conversationId'],
      replyCount: conversationMap['replyCount'],
      isRead: conversationMap['isRead'],
    );
  }).toList();
...
Future<void> storeConversations(List<Conversation> conversations) async {
  ...
  List<String> updatedEncodedConversations = storedConversations.map((conversation) {
    Map<String, dynamic> conversationMap = {
      'conversationId': conversation.conversationId,
      'replyCount': conversation.replyCount,
      'isRead': conversation.isRead,
  ...
  await prefs.setStringList('conversations', updatedEncodedConversations);
}

...
class PendingRequestsScreen extends StatelessWidget {
  ...
  // void acceptAllRequests(BuildContext context) async {
  //   String? chatHandle =
  //       SharedPrefs.getString(SharedPrefsKeys.CURRENT_USER_CHAT_HANDLE);
  //   int? avatarId = SharedPrefs.getInt(SharedPrefsKeys.CURRENT_USER_AVATAR_ID);
  //   if (chatHandle == null) {
  ...
  //     DatabaseEvent snapshot = await requestsRef
  //         .orderByChild('receiverId')
  //         .equalTo(currentUserId)
  //         .onValue
  //         .first;
  ...
  //     List<Future> updateRequests = [];

  //     requestsData.forEach((requestId, requestData) {
  //       if (requestData['status'] == 'pending') {
  //         updateRequests
  //             .add(requestsRef.child(requestId).update({'status': 'accepted'}));
  ...
  //         // Update the chat list for both the sender and receiver
  //         updateRequests.add(_initializeChat(senderId, receiverId));
  //       }
  //     });

  //     // Wait for all update operations to complete before continuing
